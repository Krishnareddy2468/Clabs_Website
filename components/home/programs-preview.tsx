		...
		<section className="bg-gradient-to-b from-indigo-50 to-white py-16 sm:py-24 md:py-28 lg:py-36">
			<div className="container mx-auto px-4 sm:px-6 lg:px-8">
				<div className="mb-10 sm:mb-12 md:mb-16 flex flex-col items-center justify-between gap-4 sm:gap-6 md:flex-row">
					...
					<Button
						variant="outline"
						className="rounded-full border-slate-700 bg-slate-800 px-5 sm:px-6 text-white hover:bg-slate-700"
						asChild
					>
						<Link href="/programs">
							View All Programs
					...
					{programs.map((program, index) => (
						...
						>
							<div className="bg-slate-800 p-5 sm:p-6 relative">
								{program.popular && (
									<span className="absolute top-3 sm:top-4 right-3 sm:right-4 rounded-full bg-indigo-500 px-2 sm:px-2.5 py-1 text-[10px] sm:text-xs font-medium text-white">
										Popular
									</span>
								)}
								<div className="flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-xl sm:rounded-2xl bg-indigo-500/20 text-indigo-400">
									<program.icon
